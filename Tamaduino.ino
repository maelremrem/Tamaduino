/* Tamaguino
 ...
 Forked by Maelremrem.xyz
 Adding Eeprom saving
...
const long loopTimerSave = 200000;
long timerSave = 0;
...
void loadGame(){
  int eeAddress = 0;
  if(EEPROM.get(eeAddress,hunger)==255||EEPROM.get(eeAddress,hunger)==512){
    //no save
    saveGameDefault();
  }
  EEPROM.get(eeAddress,hunger);
  eeAddress += sizeof(hunger);
  EEPROM.get(eeAddress,happiness);
  eeAddress += sizeof(happiness);
  EEPROM.get(eeAddress,health);
  eeAddress += sizeof(health);
  EEPROM.get(eeAddress,discipline);
  eeAddress += sizeof(discipline);
  EEPROM.get(eeAddress,weight);
  eeAddress += sizeof(weight);
  EEPROM.get(eeAddress,age);
  eeAddress += sizeof(age);
  EEPROM.get(eeAddress,poopometer);
  eeAddress += sizeof(poopometer);
  EEPROM.get(eeAddress,soundEnabled);
  eeAddress += sizeof(soundEnabled);
...
void saveGame(){
  ...
  EEPROM.put(0, hunger);
  eeAddress += sizeof(hunger);
  EEPROM.put(eeAddress,happiness);
  eeAddress += sizeof(happiness);
  EEPROM.put(eeAddress,health);
  eeAddress += sizeof(health);
  EEPROM.put(eeAddress,discipline);
  eeAddress += sizeof(discipline);
  EEPROM.put(eeAddress,weight);
  eeAddress += sizeof(weight);
  EEPROM.put(eeAddress,age);
  eeAddress += sizeof(age);
  EEPROM.put(eeAddress,poopometer);
  eeAddress += sizeof(poopometer);
  EEPROM.put(eeAddress,soundEnabled);
  eeAddress += sizeof(soundEnabled);
  EEPROM.put(eeAddress,hiScore);
}

//saveDefault
void saveGameDefault(){
  int eeAddress = 0;
  hunger=100;
  happiness=100;
  health=100;
